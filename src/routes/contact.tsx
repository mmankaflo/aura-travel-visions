...
function Contact() {
  return (
    <>
      <PageHero title="Let's start planning." subtitle="Our specialists are ready to craft your next journey." image={destCapetown} />
        ...
        <div className="container-x grid lg:grid-cols-5 gap-10">
          <div className="lg:col-span-2 space-y-6">
            ...
            <ul className="space-y-4">
              ...
              <ContactRow icon={MessageCircle} label="WhatsApp" value={SITE.whatsapp} href={SITE.whatsappHref} />
          ...
          >
            ...
            <p className="mt-1 text-sm text-muted-foreground">All enquiries answered by a real human.</p>
            <div className="mt-6 grid gap-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <Field id="c-name" label="Full name"><Input id="c-name" required /></Field>
                <Field id="c-email" label="Email"><Input id="c-email" type="email" required /></Field>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <Field id="c-phone" label="Phone"><Input id="c-phone" type="tel" /></Field>
                <Field id="c-dest" label="Destination of interest"><Input id="c-dest" placeholder="e.g. Mauritius" /></Field>
              </div>
              <Field id="c-msg" label="Message"><Textarea id="c-msg" rows={5} required placeholder="Tell us about your trip…" /></Field>
              <button type="submit" className="mt-2 inline-flex items-center justify-center rounded-full bg-gradient-to-r from-[var(--gold)] to-[var(--gold-soft)] px-7 py-3.5 text-sm font-semibold text-[var(--navy-deep)] hover:opacity-90">
                Send message
              </button>
            </div>
          </form>
...
function ContactRow({ icon: Icon, label, value, href }: { icon: any; label: string; value: string; href?: string }) {
  const content = (
    <div className="flex gap-4 items-start">
      <div className="grid h-10 w-10 shrink-0 place-items-center rounded-full bg-[var(--navy-deep)] text-[var(--gold)]"><Icon className="h-4 w-4" /></div>
