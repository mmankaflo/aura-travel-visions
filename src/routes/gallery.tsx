  ...
  head: () => ({
    meta: [
      { title: "Travel Gallery | Aura Travel & Tours" },
      {
        name: "description",
        content:
          "Visual journeys through Dubai, Mauritius, Zanzibar, Cape Town safaris and luxury resorts curated by Aura Travel & Tours.",
      },
  ...
  return (
    <>
      <PageHero
        ...
        subtitle="Snapshots from the places we love to send our travellers."
        image={destMauritius}
      />
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-5">
            {images.map((i, idx) => (
              <figure key={idx} className={`relative overflow-hidden rounded-2xl group ${i.span}`}>
                ...
                <figcaption className="absolute bottom-4 left-4 text-white font-display text-lg opacity-0 group-hover:opacity-100 transition-opacity">
                  {i.caption}
                </figcaption>
              </figure>
            ))}
