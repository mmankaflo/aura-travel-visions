    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-2xl max-h-[92vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Tell us about your business travel needs — our corporate desk will reply within 24
            hours.
          </DialogDescription>
        </DialogHeader>
        <form
          ...
          onSubmit={(event) => {
            submitEnquiryForm(
              event,
              "Corporate Travel Proposal Request",
              () => {
                setSize("");
                setFreq("");
                setTravelClass("");
        ...
        >
          ...
          <input type="hidden" name="_subject" value="Corporate Travel Proposal Request" />
          <input type="hidden" name="company_size" value={size} />
          <input type="hidden" name="travel_frequency" value={freq} />
          <input type="hidden" name="preferred_class" value={travelClass} />
          <input type="hidden" name="departing_from" value={departure} />
          <input type="hidden" name="services_required" value={services.join(", ")} />
          ...
          <section className="grid gap-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <F id="cf-company" label="Company name">
                <Input id="cf-company" name="company_name" required maxLength={120} />
              </F>
              <F id="cf-industry" label="Industry">
                <Input
                  id="cf-industry"
                  name="industry"
                  placeholder="e.g. Mining, Finance"
                  required
                />
              </F>
              ...
              <F id="cf-website" label="Company website">
                <Input id="cf-website" name="website" placeholder="https://" />
              </F>
            </div>
          </section>
          ...
          <section className="grid gap-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <F id="cf-first" label="First name">
                <Input id="cf-first" name="first_name" required maxLength={60} />
              </F>
              <F id="cf-surname" label="Surname">
                <Input id="cf-surname" name="surname" required maxLength={60} />
              </F>
              <F id="cf-position" label="Position / Job title">
                <Input id="cf-position" name="position" required maxLength={80} />
              </F>
              <F id="cf-email" label="Work email">
                <Input id="cf-email" name="email" type="email" required />
              </F>
              <F id="cf-mobile" label="Mobile number">
                <Input id="cf-mobile" name="mobile" type="tel" required />
              </F>
              <F id="cf-landline" label="Office number">
                <Input id="cf-landline" name="landline" type="tel" />
              </F>
            </div>
          </section>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <F id="cf-class" label="Preferred travel class">
                <Select value={travelClass} onValueChange={setTravelClass} required>
                  <SelectTrigger id="cf-class">
                    <SelectValue placeholder="Select class" />
                  </SelectTrigger>
                  <SelectContent>
                    {TRAVELLER_CLASS.map((o) => (
                      <SelectItem key={o} value={o}>
                        {o}
                      </SelectItem>
                    ))}
              ...
              <F id="cf-routes" label="Top destinations / routes">
                <Input
                  id="cf-routes"
                  name="top_routes"
                  placeholder="e.g. JNB–CPT, JNB–DXB, JNB–LHR"
                  required
                />
              </F>
