    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-2xl max-h-[92vh] overflow-y-auto">
        ...
        >
          ...
          <input type="hidden" name="_subject" value="Visa Application Enquiry" />
          <input type="hidden" name="visa_type" value={visaType} />
          <input type="hidden" name="purpose" value={purpose} />
          <input type="hidden" name="passport_nationality" value={passport} />
          <input type="hidden" name="contact_method" value={contactMethod} />
          <input type="hidden" name="previously_held_visa" value={previousVisa ? "Yes" : "No"} />
          <input type="hidden" name="travel_insurance_quote" value={insurance ? "Yes" : "No"} />
          <input
            type="hidden"
            name="travel_date"
            value={travelDate ? format(travelDate, "PPP") : ""}
          />
          ...
          <section className="grid gap-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <F id="vf-first" label="First name">
                <Input id="vf-first" name="first_name" required maxLength={60} />
              </F>
              <F id="vf-surname" label="Surname">
                <Input id="vf-surname" name="surname" required maxLength={60} />
              </F>
              <F id="vf-email" label="Email address">
                <Input id="vf-email" name="email" type="email" required maxLength={120} />
              </F>
              <F id="vf-mobile" label="Mobile number">
                <Input id="vf-mobile" name="mobile" type="tel" required maxLength={20} />
              </F>
              <F id="vf-passport" label="Passport nationality">
                <Select value={passport} onValueChange={setPassport} required>
                  <SelectTrigger id="vf-passport">
          ...
          <section className="grid gap-4">
            ...
            <F id="vf-prev" label="Have you held a visa for this country before?">
              <label className="flex items-center gap-3 cursor-pointer">
                <Checkbox
                  id="vf-prev"
                  checked={previousVisa}
                  onCheckedChange={(c) => setPreviousVisa(c === true)}
                />
                <span className="text-sm text-muted-foreground">
                  Yes, I've previously held a visa for this country.
                </span>
              </label>
            </F>
            ...
            <label className="flex items-start gap-3 cursor-pointer">
              ...
              <span className="text-sm">
                ...
                <span className="block text-muted-foreground text-xs">
                  Required for most visas, we'll send options.
                </span>
              </span>
            </label>
          </section>
